...
LOCAL_APPS = [
    ...
    'gabis.apps.staffs.apps.StaffsAppConfig',
]
...
QR_CODE_URL_PROTECTION = {
    ...
    constants.ALLOWS_EXTERNAL_REQUESTS_FOR_BOOKED_USER: True,  # Tells whether a registered user can request the QR code URLs from outside a site that uses this app. It can be a boolean value used for any user or a callable that takes a user as parameter. Defaults to False (nobody can access the URL without the signature token).
